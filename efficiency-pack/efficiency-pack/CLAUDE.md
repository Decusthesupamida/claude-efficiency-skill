...
- **PostToolUse / Write|Edit|MultiEdit** → `auto-checkpoint.sh` appends changed files to `.claude/edit-log.tmp`.
...
- Inspect real token cost: `jq -s 'map(.approx_tokens)|add' .claude/metrics.jsonl`.
